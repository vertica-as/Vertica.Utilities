...
namespace Vertica.Utilities_v4.Security
{
	public class BCrypt
	{
		...
		public static string HashPassword(string password, string salt)
		{
			...
			byte[] saltBytes = Base64.Decode(salt.Substring(offset + 3, 22),
											BCRYPT_SALT_LEN);
			...
			rs.Append(rounds);
			rs.Append('$');
			rs.Append(Base64.Encode(saltBytes, saltBytes.Length));
			rs.Append(Base64.Encode(hashed,
		...
		public static string GenerateSalt(int logRounds)
		{
			...
			rs.Append(logRounds);
			rs.Append('$');
			rs.Append(Base64.Encode(randomBytes, randomBytes.Length));
